...
class IndexController extends Controller {
	...
	*/
	...
	public function getNews()
	{
		...
		$tintuc = DB::table('news')->select()->where('status',1)->where('com','cong-trinh')->orderby('id','desc')->get();		
	...
	public function getListNews($alias)
	{
		...
		$tintuc_cate = DB::table('news_categories')->select()->where('status',1)->where('com','cong-trinh')->where('alias',$alias)->first();
	...
	{
		$news_detail = DB::table('news')->select()->where('status',1)->where('com','cong-trinh')->where('alias',$alias)->first();
		
		if(!empty($news_detail)){			
			$cate_pro = DB::table('news_categories')->where('status',1)->where('parent_id',0)->orderby('id','asc')->get();
			$album_hinh = DB::table('images')->select()->where('news_id',$news_detail->id)->orderby('id','asc')->get();
			$newsSameCate = DB::table('news')->select()->where('status',1)->where('com','cong-trinh')->where('cate_id',$news_detail->cate_id)->take(12)->get();	
			$com='tin-tuc';
			$setting = Cache::get('setting');
			// Cấu hình SEO
